...
# При желании цвета можно легко поменять, не перебирая во всей логике приложения.
class Color:
    yellow2 = '\033[1;35m'
    reset = '\033[0m'
    blue = '\033[0;34m'
...
# По визуальному отображению проверяем тип клетки.
...
class Field(object):
    ...
    # а все клетки корабля - уничтоженными (Cell.destroyed_ship). Так делаем, в два подхода.
    ...
    def get_max_weight_cells(self):
        ...
        # Запоминаем максимальное значение. Далее, просто берём из словаря список координат с этим
    ...
    def recalculate_weight_map(self, available_ships):
        ...
        # Необязательно знать, какой вес был у клетки в предыдущий раз:
        ...
        # Если там уничтоженный корабль или клетка с промахом - ставим туда коэффициент 0.
        ...
        # и, если он помещается, прибавляем клетке коэфф. 1.
